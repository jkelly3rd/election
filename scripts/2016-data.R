...
for (state in state_abbreviations) {
  ...
  json_url <- paste0("https://api-election.cbsnews.com/api/public/counties/2016/G/", state, "/P")
  ...
  county_candidate_data_clean_grouped <- county_candidate_data_clean %>% 
    ...
    summarise(totalExpVote = sum(totalExpVote),
              ...
              `vote_Hillary Clinton` = sum(`Hillary Clinton`),
...
all_counties_clean <- all_counties_names %>% 
  mutate(vote_Other = totalVote-(`vote_Hillary Clinton`+`vote_Donald Trump`)) %>% 
  mutate(pctExpVote = (totalVote/totalExpVote)*100) %>% 
  mutate(`pct_Hillary Clinton` = (`vote_Hillary Clinton`/totalVote)*100) %>%
  ...
  mutate(fips_new = case_when(state == "AK" ~ str_replace_all(fips, "029", "020"),
                          TRUE ~ fips)) %>% 
  mutate(leader = case_when(`pct_Hillary Clinton` > `pct_Donald Trump` ~ "Clinton",
                            `pct_Hillary Clinton` < `pct_Donald Trump` ~ "Trump",
                            TRUE ~ "NA")) %>% 
  mutate(at_least_20pct_in = case_when(pctExpVote >= 20 ~ "20pctExpVoteIn",
                                       TRUE ~ "lessThan20pctIn")) %>% 
  mutate(leader_margin = `pct_Hillary Clinton` - `pct_Donald Trump`) %>% 
...
write.csv(all_counties_clean, "output/all_counties_clean_2016.csv", row.names = FALSE)
